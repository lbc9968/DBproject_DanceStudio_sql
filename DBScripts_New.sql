...
-- -----------------------------------------------------
-- Table `DanceStudio`.`employees`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `DanceStudio`.`employees` ;
...
CREATE VIEW `DanceStudio`.`v_student_reservations` AS
    SELECT cd.class_reservation_id, s.first_name, s.last_name, dc.class_name, e.name, sr.room_name, sr.max_capacity, 
...
BEGIN
    ...
    SELECT reservation_id FROM room_reservations WHERE status_id = 1 and reservation_id != IN_class_reservation_id;
  ...
  read_loop: LOOP
    ...
    IF (CURDATE() > class_end_temp)  THEN
		UPDATE room_reservations SET status_id = 2 WHERE reservation_id = reservation_id_for_update;        
	END IF;
    SET done=FALSE;
  END LOOP;
